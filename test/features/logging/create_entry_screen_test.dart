...
void main() {
  ...
  testWidgets('renders correctly — mood slider/picker, text field, and submit button are all visible',
      (tester) async {
    // Mock the speech_to_text method channel
    const methodChannel = MethodChannel('plugin.csdcorp.com/speech_recognition');
  ...
  testWidgets('submit disabled when form is empty — submit button is disabled before the user enters anything',
      (tester) async {
    // Mock the speech_to_text method channel
    const methodChannel = MethodChannel('plugin.csdcorp.com/speech_recognition');
    TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
  ...
  testWidgets('successful create — tapping submit with valid input calls createLogEntry and navigates back',
      (tester) async {
    // Mock the speech_to_text method channel
    const methodChannel = MethodChannel('plugin.csdcorp.com/speech_recognition');
    TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
  ...
  testWidgets('failed create — when createLogEntry returns false, an error message is shown (no navigation)',
      (tester) async {
    // Mock the speech_to_text method channel
    const methodChannel = MethodChannel('plugin.csdcorp.com/speech_recognition');
    TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
    ...
    expect(find.text('Failed to create entry. Please try again.'), findsOneWidget);

    TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
        .setMockMethodCallHandler(methodChannel, null);
  });

  testWidgets('voice input button visible — VoiceInputButton is in the widget tree',
      (tester) async {
    // Mock the speech_to_text method channel
    const methodChannel = MethodChannel('plugin.csdcorp.com/speech_recognition');
    TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
