...
void main() {
  group('DashboardRepository.getInsights', () {
    ...
    test('returns empty list when there are no log entries', () async {
      when(() => loggingRepository.getLogEntries(userId))
          .thenAnswer((_) async => []);
    ...
    test('adds "Mood Improvement Detected" when recent average is higher', () async {
    ...
    test('adds "Pattern Detected" prediction when there are >= 5 mood entries', () async {
      ...
      final entries = <LogEntryModel>[
        ...
        _entry(userId: userId, date: monday.add(const Duration(days: 1)), mood: 3), // Tuesday
        _entry(userId: userId, date: monday.add(const Duration(days: 2)), mood: 3), // Wednesday
        _entry(userId: userId, date: monday.add(const Duration(days: 3)), mood: 3), // Thursday
      ];
