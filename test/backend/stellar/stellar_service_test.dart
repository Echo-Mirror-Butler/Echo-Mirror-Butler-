...
import '../../../backend/stellar/stellar_config.dart';
...
void main() {
  group('StellarService', () {
    ...
    group('createWallet()', () {
      test('returns a valid Stellar keypair with public key starting with G', () async {
      ...
      test('returns a valid Stellar keypair with secret key starting with S', () async {
        final keypair = await StellarService.createWallet();
    ...
    group('getEchoBalance()', () {
      test('returns 0.0 for a fresh unfunded wallet without trustline', () async {
      ...
      test('returns 0.0 when issuer public key is not configured', () async {
        // This test verifies the behavior when StellarConfig.issuerPublicKey is empty
    ...
    group('establishTrustline()', () {
      test('does not throw when called with valid testnet keys', () async {
        // Create issuer and user wallets
        final issuerKeypair = await StellarService.createWallet();
        ...
        // Since StellarConfig.issuerPublicKey is const, we test the method
        // doesn't throw with the current configuration
        expect(
          () async => await StellarService.establishTrustline(userKeypair.secretSeed),
          returnsNormally,
        );
      });
      ...
      test('returns false when issuer public key is not configured', () async {
        ...
        final result = await StellarService.establishTrustline(userKeypair.secretSeed);
    ...
    group('sendEcho()', () {
      ...
      test('handles long memo by truncating to 28 characters', () async {
        ...
        final longMemo = 'This is a very long memo that exceeds twenty eight characters';
    ...
    group('End-to-end ECHO transfer flow', () {
      test('complete flow: create wallets, establish trustline, and verify balance', () async {
        ...
        // Establish trustline manually (since StellarService needs env config)
        ...
        final updatedAccount = await sdk.accounts.account(userKeypair.accountId);
        final echoBalance = updatedAccount.balances.firstWhere(
          (b) => b.assetCode == EchoToken.code && b.assetIssuer == issuerKeypair.accountId,
      ...
      test('sendEcho transfers correct amount between wallets', () async {
        // Create issuer
        final issuerKeypair = KeyPair.random();
        await http.get(Uri.parse('$friendbotUrl?addr=${issuerKeypair.accountId}'));
        ...
        // Fund both wallets
        await http.get(Uri.parse('$friendbotUrl?addr=${senderKeypair.accountId}'));
        await http.get(Uri.parse('$friendbotUrl?addr=${recipientKeypair.accountId}'));
        ...
        final senderTrustTx = TransactionBuilder(senderAccount)
            .addOperation(ChangeTrustOperationBuilder(echoAsset, '1000000').build())
            .build();
        senderTrustTx.sign(senderKeypair, network);
        await sdk.submitTransaction(senderTrustTx);

        // Recipient trustline
        final recipientAccount = await sdk.accounts.account(recipientKeypair.accountId);
        ...
        final senderAccountUpdated = await sdk.accounts.account(senderKeypair.accountId);
        ...
        // Verify balances
        final finalSenderAccount = await sdk.accounts.account(senderKeypair.accountId);
        final finalRecipientAccount = await sdk.accounts.account(recipientKeypair.accountId);
        ...
        expect(double.tryParse(senderEchoBalance.balance), closeTo(450.0, 0.01));
    ...
    group('Error handling', () {
      test('handles invalid secret key gracefully', () async {
        ...
        final invalidSecret = 'SINVALIDSECRETKEY1234567890123456789012345678901234567890';
        
        // Should not throw but return false/null
        final trustlineResult = await StellarService.establishTrustline(invalidSecret);
      ...
      test('handles non-existent account gracefully', () async {
        final nonExistentPublicKey = 'GAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA';
        
        final balance = await StellarService.getEchoBalance(nonExistentPublicKey);
        expect(balance, equals(0.0));
      });
    });
  });
