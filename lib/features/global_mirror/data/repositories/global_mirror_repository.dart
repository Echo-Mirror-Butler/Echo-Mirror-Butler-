...
class GlobalMirrorRepository {
        ...
        while (true) {
          try {
            await for (final pins in _client.global.streamMoodPins()) {
              ...
              debugPrint('[GlobalMirrorRepository] Received ${mappedPins.length} mood pins (attempt $attemptCount)');
              yield mappedPins;
            }
          } catch (e, stackTrace) {
            ...
            await Future.delayed(const Duration(seconds: 5));
            debugPrint('[GlobalMirrorRepository] Retrying stream connection...');
          }
        }
      } catch (e, stackTrace) {
  ...
  Future<String?> addMoodPin({
    ...
    try {
      ...
      if (_useMockData) {
        ...
        _mockPins.add(pin);
        debugPrint('[GlobalMirrorRepository] Mock pin added: $sentiment at ($gridLat, $gridLon)');
        return pinId;
      } else {
        // Call Serverpod endpoint (returns pin ID)
        try {
          debugPrint('[GlobalMirrorRepository] Adding mood pin: $sentiment at ($latitude, $longitude) -> ($gridLat, $gridLon)');
          ...
          debugPrint('[GlobalMirrorRepository] Added mood pin: $sentiment at ($gridLat, $gridLon) - Pin ID: $pinId');
          if (pinId != null) {
            debugPrint('[GlobalMirrorRepository] Mood pin successfully added to database. It should appear in the stream within 5 seconds.');
          } else {
            debugPrint('[GlobalMirrorRepository] WARNING: addMoodPin returned null - pin may not have been saved');
          }
          return pinId;
        } catch (e, stackTrace) {
          debugPrint('[GlobalMirrorRepository] Error adding mood pin to server: $e');
          debugPrint('[GlobalMirrorRepository] Stack trace: $stackTrace');
          return null;
        }
      }
  ...
  Future<String?> uploadVideo({
    ...
    try {
      ...
      if (!await file.exists()) {
        debugPrint('[GlobalMirrorRepository] ERROR: Video file does not exist at path: $videoPath');
        return null;
      }
      
      // Check file size
      final fileSize = await file.length();
      debugPrint('[GlobalMirrorRepository] Video file size: ${fileSize / (1024 * 1024)} MB');
      ...
      if (_useMockData) {
        ...
        debugPrint('[GlobalMirrorRepository] Mock video uploaded successfully: $moodTag');
        ...
        try {
          ...
          if (fileSize > maxDirectUploadSize) {
            debugPrint('[GlobalMirrorRepository] ERROR: Video file (${(fileSize / (1024 * 1024)).toStringAsFixed(2)} MB / ${(fileSize / 1024).toStringAsFixed(0)} KB) exceeds safe upload limit of 400KB');
            debugPrint('[GlobalMirrorRepository] Serverpod endpoint limit is 512KB, but request serialization adds significant overhead (~20-25%)');
            debugPrint('[GlobalMirrorRepository] Please use a video smaller than 400KB (typically 8-10 seconds), enable mock data mode for testing, or implement FileUploader on server for larger files');
            return null;
          }
          
          debugPrint('[GlobalMirrorRepository] Reading video file bytes...');
          final bytes = await file.readAsBytes();
          debugPrint('[GlobalMirrorRepository] Read ${bytes.length} bytes from video file');
          
          final byteData = ByteData.view(bytes.buffer);
          
          debugPrint('[GlobalMirrorRepository] Uploading to Serverpod...');
          final videoUrl = await _client.global.uploadVideo(
            byteData,
            moodTag,
          );
          
          if (videoUrl != null && videoUrl.isNotEmpty) {
            debugPrint('[GlobalMirrorRepository] Video uploaded successfully: $moodTag - URL: $videoUrl');
          } else {
            debugPrint('[GlobalMirrorRepository] WARNING: Upload returned null or empty URL');
          }
          return videoUrl;
        } catch (e, stackTrace) {
          debugPrint('[GlobalMirrorRepository] Error uploading video to server: $e');
          debugPrint('[GlobalMirrorRepository] Stack trace: $stackTrace');
          ...
          if (e.toString().contains('Request size exceeds') || 
            ...
            debugPrint('[GlobalMirrorRepository] Consider: 1) Using a smaller video (<500KB), 2) Implementing FileUploader on server, or 3) Using mock data mode');
          }
    ...
    try {
      if (_useMockData) {
        ...
        try {
          ...
          debugPrint('[GlobalMirrorRepository] Error getting video feed from server: $e');
          return [];
        }
      }
    } catch (e) {
  ...
  Future<Position?> getCurrentLocation() async {
    try {
      ...
      if (permission == LocationPermission.deniedForever) {
        debugPrint('[GlobalMirrorRepository] Location permission denied forever');
        return null;
      }
      ...
      return await Geolocator.getCurrentPosition(
        locationSettings: const LocationSettings(
          accuracy: LocationAccuracy.low, // Low accuracy is fine for anonymized data
        ),
      );
    } catch (e) {
  ...
  void _initializeMockData() {
    ...
    final sentiments = ['positive', 'neutral', 'negative', 'excited', 'calm', 'anxious'];
    final locations = [
  ...
  void _initializeMockVideos() {
    ...
    for (var i = 0; i < 15; i++) {
      _mockVideos.add(VideoPostModel(
        id: 'mock_video_$i',
  ...
  Future<String?> addComment({
    ...
    try {
      ...
      if (_useMockData) {
        ...
        // Call Serverpod endpoint
        try {
          debugPrint('[GlobalMirrorRepository] Adding comment to mood pin: $moodPinId');
          final commentIdInt = await _client.global.addComment(
            pinIdInt,
            text,
          );
          ...
          debugPrint('[GlobalMirrorRepository] Error adding comment to server: $e');
          debugPrint('[GlobalMirrorRepository] Stack trace: $stackTrace');
          return null;
        }
      }
    ...
    try {
      ...
      if (_useMockData) {
        ...
        try {
          ...
          debugPrint('[GlobalMirrorRepository] Error getting comments from server: $e');
          debugPrint('[GlobalMirrorRepository] Stack trace: $stackTrace');
          return [];
        }
      }
