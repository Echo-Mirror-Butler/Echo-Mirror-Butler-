...
exports.stellarRouter.use(auth_1.requireAuth);
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
exports.stellarRouter.post('/wallet/create', async (req, res, next) => {
    ...
    try {
        ...
        const { publicKey, secretKey } = await (0, stellar_1.createWallet)();
        await (0, stellar_1.establishTrustline)(secretKey);
        const { error } = await supabase_1.supabase.from('user_wallets').insert({
            user_id: userId,
...
// ─────────────────────────────────────────────
exports.stellarRouter.get('/wallet/balance', async (req, res, next) => {
    const userId = req.userId;
    try {
        ...
        const balances = await (0, stellar_1.getWalletBalances)(wallet.public_key);
...
// ─────────────────────────────────────────────
exports.stellarRouter.post('/gift', async (req, res, next) => {
    ...
    try {
        ...
        const balances = await (0, stellar_1.getWalletBalances)(senderWallet.data.public_key);
        ...
        const result = await (0, stellar_1.sendEcho)(senderWallet.data.secret_key, recipientWallet.data.public_key, parsedAmount.toFixed(7), message);
        ...
        const stellarError = err;
        await supabase_1.supabase.from('gift_transactions').insert({
            sender_id: senderId,
            recipient_id: recipientUserId,
            amount: parsedAmount,
            tx_hash: null,
        ...
        if (stellarError.message?.includes('op_no_trust') ||
...
// ─────────────────────────────────────────────
exports.stellarRouter.get('/transactions', async (req, res, next) => {
    const userId = req.userId;
