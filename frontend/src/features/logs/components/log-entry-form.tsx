...
export type LogEntryFormProps = {
  mode: LogFormMode
  id?: string
  initialDate?: string
  onSuccess?: (mode: LogFormMode) => void
  onCancel?: () => void
  onExistingFound?: (id: string) => void
...
export function LogEntryForm({ mode, id, initialDate, onSuccess, onCancel, onExistingFound, onDeleteSuccess }: LogEntryFormProps) {
  const { user } = useAuth()
  const queryClient = useQueryClient()
  const { showToast } = useToast()

  const defaultDate = initialDate ?? toDateInputValue(new Date())
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: async (data) => {
      if (!data && mode === 'create') return // handled by onExistingFound
