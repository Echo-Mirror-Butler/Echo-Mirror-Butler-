...
class _EndpointGift extends EndpointRef {
  ...
  Future<double> getEchoBalance() =>
      caller.callServerEndpoint<double>('gift', 'getEchoBalance', {});

  Future<GiftTransaction?> sendGift(
    int recipientUserId,
    double amount,
    String? message,
  ) => caller.callServerEndpoint<GiftTransaction?>('gift', 'sendGift', {
    'recipientUserId': recipientUserId,
    'amount': amount,
    'message': message,
  });

  Future<List<GiftTransaction>> getGiftHistory() => caller
      .callServerEndpoint<List<GiftTransaction>>('gift', 'getGiftHistory', {});

  Future<double> awardEcho(int userId, double amount, String reason) =>
      caller.callServerEndpoint<double>('gift', 'awardEcho', {
        'userId': userId,
        'amount': amount,
        'reason': reason,
      });
...
class _EndpointGlobal extends EndpointRef {
  ...
  Future<int?> addMoodPin(
    ...
    double latitude,
    double longitude,
  ) => caller.callServerEndpoint<int?>('global', 'addMoodPin', {
    'sentiment': sentiment,
    'latitude': latitude,
    'longitude': longitude,
  });

  Future<String?> uploadVideo(ByteData videoData, String moodTag) =>
      caller.callServerEndpoint<String?>('global', 'uploadVideo', {
        'videoData': videoData,
        'moodTag': moodTag,
      });

  Future<String?> uploadImage(ByteData imageData, String moodTag) =>
      caller.callServerEndpoint<String?>('global', 'uploadImage', {
        'imageData': imageData,
        'moodTag': moodTag,
      });

  Future<List<VideoPost>> getVideoFeed(int offset, int limit) =>
      caller.callServerEndpoint<List<VideoPost>>('global', 'getVideoFeed', {
        'offset': offset,
        'limit': limit,
      });

  Future<int?> addComment(int moodPinId, String text) =>
      caller.callServerEndpoint<int?>('global', 'addComment', {
        'moodPinId': moodPinId,
        'text': text,
      });
  ...
  Future<String> generateClusterEncouragement(
    ...
    {'sentiment': sentiment, 'nearbyCount': nearbyCount},
  );
}
...
class _EndpointPasswordReset extends EndpointRef {
  ...
  Future<bool> requestPasswordReset(String email) =>
      caller.callServerEndpoint<bool>('passwordReset', 'requestPasswordReset', {
        'email': email,
      });

  Future<bool> resetPassword(String email, String token, String newPassword) =>
      caller.callServerEndpoint<bool>('passwordReset', 'resetPassword', {
        'email': email,
        'token': token,
  ...
  Future<bool> changePassword(String currentPassword, String newPassword) =>
      caller.callServerEndpoint<bool>('passwordReset', 'changePassword', {
...
class Client extends ServerpodClientShared {
  Client(
    ...
    Function(MethodCallContext context, Object error, StackTrace stackTrace)?
    onFailedCall,
    Function(CloseReason reason)? onSuccessfulStreamClose,
    Function(SessionLogEntry entry)? logReceivedCall,
  }) : super(
         host,
         Protocol(),
